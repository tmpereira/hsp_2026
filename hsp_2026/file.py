...
def _fsm_decode_5100(data):
    '''
    Bloco 5100: metadados principais do mapeamento.
    Contem dimensoes (n_x, n_y, n_z) e intervalo espectral (z_start, z_end, z_delta).
    '''
    name_size = struct.unpack('<h', data[:2])[0]
    name = data[2:name_size + 2].decode('utf8', errors='replace')
...
def _fsm_decode_5104(data):
    '''Bloco 5104: informacoes do instrumento (analyst, date, modelo, etc.).'''
    text = []
    i = 0
    while i + 2 < len(data):
        tag = data[i:i + 2]
...
def _fsm_decode_5105(data):
    '''Bloco 5105: dados espectrais brutos (float32).'''
    return np.frombuffer(data, dtype=np.float32)


_FSM_DECODERS = {
    5100: _fsm_decode_5100,
    5104: _fsm_decode_5104,
    5105: _fsm_decode_5105,
}


def _read_fsm_binary(path):
    '''
    Le o arquivo FSM binario e retorna (amplitudes, wavelength, meta).

    amplitudes : ndarray float32  shape (n_espectros, n_pontos)
    wavelength : ndarray float64  shape (n_pontos,)
    meta       : dict com n_x, n_y, n_z, filename, etc.
    '''
    with open(path, 'rb') as f:
        content = f.read()
    ...
    if content[:4] != b'PEPE':
        raise ValueError(
            'Arquivo nao e um FSM valido — assinatura esperada: PEPE\n'
            'Arquivo: ' + path
        )
    ...
    # --- montar array de amplitudes ---
    amplitudes = np.squeeze(np.array(spectral_blocks, dtype=np.float32))

    # --- construir vetor de numeros de onda ---
    wavelength = np.arange(
        meta['z_start'],
        meta['z_end'] + meta['z_delta'],
        meta['z_delta'],
    ...
    # garantir que o tamanho bate com n_z
    if 'n_z' in meta:
        wavelength = wavelength[:meta['n_z']]
...
# ─────────────────────────────────────────────────────────────────────────────
# API PUBLICA — mesma interface do hsp_2024
# ─────────────────────────────────────────────────────────────────────────────

def fsm(arq):
    '''
    Le um arquivo FSM da Perkin Elmer Spotlight.
    Leitura nativa — sem dependencia do specio.

    Parametros
    ----------
    arq : str — caminho para o arquivo .fsm

    Retorna
    -------
    data : dict hsp com chaves:
        r        — array float32 (n_espectros x n_pontos), absorbancia (-log10)
        wn       — array float64 (n_pontos,), numeros de onda cm-1
        dx, dy   — dimensoes espaciais da imagem
        sel      — mascara booleana (todos True na leitura)
        filename — nome do arquivo
        log      — string de log
    '''
    amplitudes, wavelength, meta = _read_fsm_binary(arq)

    # no FSM da Perkin Elmer:
    #   n_x = numero de colunas -> dy na convencao da imagem (eixo horizontal)
    #   n_y = numero de linhas  -> dx na convencao da imagem (eixo vertical)
    n_x = int(meta.get('n_x', 1))
    n_y = int(meta.get('n_y', 1))
    dx  = n_y   # linhas
    dy  = n_x   # colunas

    # garantir shape 2D (n_espectros x n_pontos)
    if amplitudes.ndim == 1:
        amplitudes = amplitudes.reshape(1, -1)

    # conversao para absorbancia: A = -log10(0.01 * T%)
    # (mesma transformacao do hsp_2024 original)
    r = np.fliplr(amplitudes.astype('float32'))
    r = -np.log10(np.clip(0.01 * r, 1e-10, None))  # clip evita log(0)
    ...
    data = {
        ...
        'dy':       np.array(dy),
        'filename': np.array(meta.get('filename', os.path.basename(arq))),
        'sel':      np.ones(r.shape[0], dtype=bool),
        'log':      np.array('abrindo arquivo FSM: ' + os.path.basename(arq)),
    }
...
def get_fsm_files(path):
    '''Retorna lista de todos os arquivos .fsm em um diretorio.'''
    os.chdir(path)
    return [os.path.join(path, f) for f in glob.glob('*.fsm')]
...
def age(path):
    '''
    Le um mosaico Agilent a partir de arquivos .dmd e .dmt.
    Sem alteracoes em relacao ao hsp_2024 — I/O bound, mantido em numpy.
    '''
    dx_list, dy_list = [], []
    for fname in glob.glob(path + '*.dmd'):
        parts = fname[-13:-4].split('_')
    ...
    # ler vetor de numeros de onda do .dmt
    arq_dmt = glob.glob(path + '*.dmt')[0]
    tmp_i4  = np.fromfile(arq_dmt, dtype='i4')
    npoints = tmp_i4[559]
    start   = tmp_i4[557]
